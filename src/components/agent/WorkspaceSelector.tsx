...
export function WorkspaceSelector(): React.ReactElement {
  ...
  const handleCreate = async (): Promise<void> => {
    ...
    try {
      const workspace = await ipc.createAgentWorkspace(trimmed)
  ...
  const handleRename = async (): Promise<void> => {
    ...
    try {
      const updated = await ipc.updateAgentWorkspace(editingId, { name: trimmed })
  ...
  const handleDrop = (e: React.DragEvent, targetId: string): void => {
    ...
    ipc.reorderAgentWorkspaces(reordered.map((w) => w.id)).catch(console.error)
  }
