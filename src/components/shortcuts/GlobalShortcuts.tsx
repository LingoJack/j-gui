...
export function GlobalShortcuts(): null {
  ...
  useEffect(() => {
    const cleanup = ipc.onQuickTaskOpenSession(async (data) => {
      try {
        // 切换到对应模式
        ...
        if (data.mode === 'agent') {
          ...
          if (data.files && data.files.length > 0) {
            for (const file of data.files) {
              try {
                const result = await ipc.saveAttachment({
  ...
  useEffect(() => {
    const cleanup = ipc.onVoiceDictationInsertText(({ text }) => {
      const trimmed = text.trim()
      if (!trimmed) return

      const insertedAtCursor = !window.dispatchEvent(new CustomEvent('jgui:insert-text', {
        cancelable: true,
        detail: { text: trimmed },
      }))
      if (insertedAtCursor) {
