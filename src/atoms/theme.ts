 ...
 * - themeModeAtom: 用户选择的主题模式，持久化到本地数据目录
...
/** localStorage 缓存键 */
const THEME_CACHE_KEY = 'jgui-theme-mode'
const THEME_STYLE_CACHE_KEY = 'jgui-theme-style'
...
export async function initializeTheme(
  ...
  // 获取系统主题
  const isDark = await ipc.getSystemTheme()
  setSystemIsDark(isDark)

  // 监听系统主题变化
  const cleanupSystem = ipc.onSystemThemeChanged((newIsDark) => {
    setSystemIsDark(newIsDark)
  })

  // 监听用户手动切换主题（跨窗口同步，如 Quick Task 面板）
  const cleanupThemeSettings = ipc.onThemeSettingsChanged((payload) => {
...
export async function updateThemeMode(mode: ThemeMode): Promise<void> {
  cacheThemeMode(mode)
  await ipc.updateSettings({ themeMode: mode })
}
...
export async function updateThemeStyle(style: ThemeStyle): Promise<void> {
  cacheThemeStyle(style)
  await ipc.updateSettings({ themeStyle: style })
}
