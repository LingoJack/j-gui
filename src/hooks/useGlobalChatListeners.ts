...
export function useGlobalChatListeners(): void {
  ...
  useEffect(() => {
    ...
    // ===== 2. 流式推理内容 =====
    const cleanupReasoning = ipc.onStreamReasoning(
    ...
    // ===== 3. 流式完成 =====
    const cleanupComplete = ipc.onStreamComplete(
    ...
    // ===== 4. 流式错误 =====
    const cleanupError = ipc.onStreamError(
    ...
    // ===== 5. 工具活动 =====
    const cleanupToolActivity = ipc.onStreamToolActivity(
