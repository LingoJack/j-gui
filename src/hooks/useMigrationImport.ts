...
export function useMigrationImport(initialFilePath?: string | null): UseMigrationImportReturn {
  ...
  const parseFile = useCallback(async (filePath: string) => {
    ...
    try {
      const preview = await ipc.migrationParseImportFile(filePath) as ImportPreview
  ...
  const handleSelectImportFile = useCallback(async () => {
    ...
    try {
      const filePath = await ipc.migrationOpenFileDialog()
      if (!filePath) {
        setImporting(false)
        return
      }
  ...
  const handleConfirmImport = useCallback(async () => {
    ...
    try {
      await ipc.migrationConfirmImport({
