...
export interface AgentSessionMeta {
  ...
  /** 分叉来源：源会话的 j-gui 工作目录（用于保留历史来源边界，当前不自动驱动 SDK resume） */
  forkSourceDir?: string;
  /** 分叉来源：源会话的 SDK session ID（仅记录恢复边界，当前不自动恢复隐藏上下文） */
  forkSourceSdkSessionId?: string;
  /** 回退后的可见时间线截断点（当前只记录为边界元数据，不自动驱动 SDK resume） */
...
/** Agent 运行后端模式 */
export type AgentBackendMode = "claude-sdk" | "jagent";
...
export interface AgentSendInput {
  ...
  /** Agent 后端模式：Claude Code CLI/SDK 或 j-cli agent loop */
  backendMode?: AgentBackendMode;
...
/** 权限模式定义顺序（用于循环切换） */
export const JGUI_PERMISSION_MODE_ORDER: readonly JguiPermissionMode[] = [
  "auto",
  "bypassPermissions",
  "plan",
];
...
export function migratePermissionMode(mode: string): JguiPermissionMode {
  if (mode === "auto" || mode === "bypassPermissions" || mode === "plan")
    return mode;
